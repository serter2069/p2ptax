  ...
  useEffect(() => {
    ...
    async function load() {
      try {
        ...
        const latest = items.find(i => i.viewport === 'desktop');
        if (!latest) { setError('No desktop mosaic found. Run: metromap mosaic p2ptax --desktop'); return; }
  ...
  return (
    ...
    <div style={{background:'#111',minHeight:'100vh',fontFamily:'system-ui,sans-serif'}}>
      ...
      <div style={{position:'sticky',top:0,zIndex:10,background:'#1a1a1a',borderBottom:'1px solid #333',padding:'8px 16px',display:'flex',alignItems:'center',justifyContent:'space-between'}}>
        {/* @ts-ignore */}
        <span style={{color:'#fff',fontWeight:600,fontSize:15}}>Desktop Mosaic</span>
        {/* @ts-ignore */}
