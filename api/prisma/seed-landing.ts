...
const SPECIALIST_IDS = {
  ...
  vladimir:"45790a39-0285-48d7-943c-bea399edc3f1", // Владимир Лебедев → Выездная проверка
...
async function main() {
  ...
  // 2. Владимир Лебедев — set ONLY Выездная проверка
  console.log("Владимир Лебедев: setting Выездная проверка…");
  await setOnlyService(SPECIALIST_IDS.vladimir, SERVICE_IDS.vyezdnaya);
  await prisma.user.update({
  ...
  // 3. Светлана Орлова — set ONLY Отдел оперативного контроля
  console.log("Светлана Орлова: setting Отдел оперативного контроля…");
  ...
  console.log("Светлана Орлова: done");
